...
class LogisticRegression:
    def __init__(self):
        ...
        self.b = tf.Variable(tf.zeros([self.dim_output]), name="bias")
#        self.y = tf.nn.softmax(tf.matmul(self.x,W)+b, name="output")
        self.y = tf.nn.sigmoid(tf.matmul(self.x,W)+self.b, name="output")
    ...
    def fit(self, data_x, data_t):
        ...
        for i in range(max_epoch):
            ...
            errs.append(err)
            if i%100==0:
                print (self.sess.run(self.b))
                print (self.sess.run(self.y, feed_dict={self.x:data_x}))
